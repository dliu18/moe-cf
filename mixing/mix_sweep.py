#!/usr/bin/env python3
"""Run baseline + random group-mixing trials on a dataset validation split."""
...
from data_preproc.movielens import ensure_splits_and_labels, _resolve
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LightGCN data-mixing sweep.")
    parser.add_argument(
        "--dataset",
        type=str,
        default="ml-1m",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "CSV output path (appends trial-by-trial). "
            "Default: mixing/validation/<dataset>/<model>/<recdim>/mixing_results.csv"
        ),
    )
    parser.add_argument(
        "--lightgcn-code-dir",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for LightGCN dataset files. Default: LightGCN/data/<dataset>",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output image path for smoke-test alpha_mix distribution plot. "
            "Default: mixing/validation/<dataset>/<model>/<recdim>/mix_sweep_smoke.png"
        ),
    )
...
def main() -> None:
    ...
    default_dataset_output = Path("LightGCN") / "data" / dataset_name
    csv_path = args.csv_path if args.csv_path is not None else default_csv
    smoke_plot_path = args.smoke_plot_path if args.smoke_plot_path is not None else default_smoke
    dataset_output_dir = args.output_dir if args.output_dir is not None else default_dataset_output

    run_mixing_sweep(
        repo_root=repo_root,
        data_dir=_resolve(repo_root, args.data_dir),
        output_dir=_resolve(repo_root, dataset_output_dir),
        dataset_name=dataset_name,
        ...
        epochs=args.epochs,
        layer=args.layer,
        csv_path=_resolve(repo_root, csv_path),
        ...
        smoke_plot_path=_resolve(repo_root, smoke_plot_path),
    )
