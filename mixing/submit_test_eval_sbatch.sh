...
for label in "${labels[@]}"; do
  sbatch \
    ...
    --wrap='mamba run -n moe-cf python mixing/evaluate_top_mixes_test.py --dataset ${DATASET} --feature-name Age --source-label ${SRC_LABEL} --model ${MODEL} --lr ${LR} --decay ${DECAY} --recdim ${RECDIM} --top-k 300 --epochs 40 --layer 1'
done
